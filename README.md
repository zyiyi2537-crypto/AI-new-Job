...
> 当前为可运行的本地 MVP。桌面模式可在工作台内直接打开 BOSS 直聘、智联招聘、猎聘和拉勾，并由用户触发采集当前岗位或搜索列表；Chrome 扩展、公开岗位 URL、手工 JD 和 JSON 仍作为备用入口。系统不会绕过验证、自动提交投递或自动向 HR 发送消息。
...
1. 选择 BOSS、智联、猎聘或拉勾。
2. 在内置页面中正常登录、搜索并打开岗位。
3. 点击“采集当前岗位”或“扫描当前列表”。
4. API 确认入库后，岗位会出现在岗位库中。
